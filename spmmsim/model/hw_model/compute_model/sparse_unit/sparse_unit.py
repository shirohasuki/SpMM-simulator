...
from reorder import SparseReorder
...
class SparseUnit: 
    ...
    def one_dim_skip(self, sparse_matrix_a, dense_matrix_b, systolic_row, systolic_col):
        ...
        A_process_matrix = np.full([len(A_access_seq), 
                                    len(max(A_access_seq, key = lambda x: len(x)))], fill_value=-1)
        for i, j in enumerate(A_access_seq):
        ...
        A_access_seq, B_access_seq = align.aligned_access(tiled_matmul_a, B_access_seq)
        # print(B_access_seq)
        # ============================================================================= 
        # generate skip access message matrix
        A_skip_access_message_matrix=[]
        B_skip_access_message_matrix=[]
        for (PE_id, tile_a) in A_access_seq:
            # print(tile_a)
            A_skip_access_message_matrix.append(SparseAddrConvert(sparse_matrix_a.shape[0], 
                sparse_matrix_a.shape[1]).gate_to_skip_message(tile_a.tolist()))
         
        # print(sparse_matrix_a.shape[0], sparse_matrix_a.shape[1])
        # for (PE_id, tile_b) in B_access_seq:
            # print(tile_b)

        # B_access_seq[0] PE_id [1] tile_b
        # 提取每个元组的第二个元素(tile_b)组成新列表
        tile_b_list = [item[1] for item in B_access_seq]
        # print(tile_b_list)
